...
for g in font.glyphs():
    ...
    if g.width > 0 and category not in ['Mn', 'Mc', 'Me']:
        ...
        if g.width != target_width:
            print("before: left: {}; right: {}; width: {}".format(g.left_side_bearing, g.right_side_bearing, g.width))
            delta = target_width - g.width
            g.left_side_bearing = int(g.left_side_bearing + delta)
            g.right_side_bearing = int(g.right_side_bearing + delta - g.left_side_bearing)
            g.width = target_width
            print("after: left: {}; right: {}; width: {}".format(g.left_side_bearing, g.right_side_bearing, g.width))
